...
Параллельно в репозитории есть **вспомогательный прототип на Python**
(`focal_interpreter/`). Он полезен для разработки, но не является нормативным
oracle. Конечный продукт и источник семантической истины — ASM-интерпретатор,
проверяемый на настоящем RARS 1.6.
...
| `tools/run_rars_profile_tests.py` | финальная нормативная приёмка TZ v1.2 |
...
Финальная автоматическая приёмка (после задания `RARS_JAR`):

```powershell
py -3 -B tools/run_rars_profile_tests.py
```

Она использует Python только как драйвер: FOCAL компилируется и исполняется
внутри RARS, а результаты сравниваются с заранее заданным acceptance manifest.

Основные документы:

- `RARS_QUICK_CHECK.md` — короткая инструкция для ручной проверки в RARS.
- `FOCAL_USER_GUIDE.md` — синтаксис и работа с нормативным RARS-интерпретатором.
