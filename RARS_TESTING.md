...
RUN/SAVE используют тот же обход и formatter, но пока сохраняют ограниченный
...
- RUN/SAVE всё ещё ограничены 8191 байтом сериализованного исходника плюс
  NUL; при превышении выдают ошибку, хотя LIST/WRITE доступны для всей программы;
- новый LOAD/SAVE относится к следующим этапам; сохранение файла пока
  не транзакционное;
